...
class SymbolicType(object):
	def __init__(self, name, expr=None):
		print ("\033[1;33;40m","type","\033[0m")
		self.name = name
		self.expr = expr
		print("symty name : ",name)
		print("symty expr : ",expr)
	...
	# public funs
	# TO DO ,symbol in left, symbol in right, multi symbol
	def rgetConcrValue(self):
		if self.isVariable():
			return self.val
		else:
			return self._rgetConcrValue(self.expr)

	def _rgetConcrValue(self,expr):
		if isinstance(expr,list):
		# might have bugs
			if len(expr) == 2: return self._rgetConcrValue(expr[1])
			return self._rgetConcrValue(expr[1])
		elif isinstance(expr,SymbolicType):
			return expr.rgetConcrValue()
		else:
			return self.val
	...
	def rgetName(self):
		if self.isVariable():
			...
			return self._rgetName(self.expr)

	def _rgetName(self,expr):
		if isinstance(expr,list):
			return self._rgetName(expr[1])
		elif isinstance(expr,SymbolicType):
			return expr.rgetName()
		else:
			return self.name
	...
	def _do_sexpr(self,args,fun,op,wrap):
		...
		print ("symty _do_sexpr fun : ",fun)
		print ("symty _do_sexpr self : ",self.__dict__)
		concrete = fun(**dict([a for a in args]))
		print ("symty _do_sexpr 4")
		symbolic = [ op ] + [ s for c,s in unwrapped ]
		print ("symty _do_sexpr 2")
